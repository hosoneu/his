...
public interface WorkLoadMapper {
    ...
    @Select({"SELECT expense_type.Expense_Type_Name,SUM(expense_items.Total_Cost) " ,
            ...
            "WHERE invoice.Invoice_ID=expense_items.Invoice_ID AND expense_items.Expense_Type_ID = expense_type.Expense_Type_ID AND invoice.User_ID = #{userID} AND expense_items.Expense_Items_ID IN (SELECT examination_fmedical_items.Expense_Items_ID " ,
            "FROM examination ,examination_fmedical_items " ,
